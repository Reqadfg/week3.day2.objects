...
heroCreator.createCharSheet = function (heroObj) {
    //Create heroContainer and give classname
    let heroContainer = document.createElement(`div`);
    heroContainer.className = `hero_container`;

    //Create image element and put in img source
    let heroImg = document.createElement(`img`);
    heroImg.src = heroObj.img;

    heroContainer.appendChild(heroImg);

    console.log(heroContainer)

    let infoDiv = this.createInfoDiv(heroObj);
    heroContainer.appendChild(infoDiv);
...
heroCreator.createInfoDiv = function (heroObj) {
    ...
    heroTextContainer.className = `hero_text_container`;

    //Create array heroObj keys
    let heroDetails = Object.keys(heroObj);

    //Loop through each key
    for (const detail of heroDetails) {
        let detailP = document.createElement(`p`);
        console.log(heroObj[detail]);

        if (Array.isArray(heroObj[detail])) {
            detailP.innerHTML = this.getAbilityString(heroObj[detail]);
            heroTextContainer.appendChild(detailP);
        }
            ...
            detailP.innerHTML = `${detail}: ${heroObj[detail]}`;
            heroTextContainer.appendChild(detailP);
        }
 ...
 * @return {String}
 */
heroCreator.getAbilityString = function (abilities) {
    let nameArray = [];
    for (const ability of abilities) {
        nameArray.push(ability.name);
    }
